...
class BaseBuilder {
  ...
  /** select query
   * @param {Array<String>} fields an optional array of strings
  ...
  select(fields = []) {
    ...
    return `SELECT ${fields.join(', ')} FROM ${this.tableName};`;
  }

  selectDistinct(fields = []) {
    if (fields.length === 0) {
      return `SELECT DISTINCT * FROM ${this.tableName};`;
    }
    return `SELECT DISTINCT ${fields.join(', ')} FROM ${this.tableName};`
...
module.exports = new BaseBuilder('users');